...
function AuthPage({ onBackToHome }) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (otpResult.success) {
          ...
          setSuccessMessage('Registration successful! Please try to verify your email.');
        }
      } else {
        // Registration failed
  ...
  const handleEmailVerification = async (e) => {
    ...
    try {
      console.log("Verifying email OTP:", { email: verificationEmail, otp: verificationCode });
      
      const result = await verifyEmailOTP(verificationEmail, verificationCode);
      
      if (result.success) {
        // Verification successful
        setSuccessMessage('Email verified successfully! Your account has been activated. You can now log in.');
        ...
        setErrors({ verificationCode: result.message || 'Invalid verification code. Please try again.' });
      }
  ...
  const handleResendCode = async () => {
    try {
      ...
      if (result.success) {
        ...
        setErrors({ submit: result.message || 'Failed to resend code. Please try again.' });
      }
